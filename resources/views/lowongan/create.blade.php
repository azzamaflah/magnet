                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <div class="flex items-center gap-2 mb-0.5">
                            ...
                            <span class="text-xs font-semibold text-[#e88968]">Tambah Baru</span>
                        </div>
            ...
            <div class="bg-[#2a2a2a]/60 backdrop-blur-md border border-[#3a3a3a] rounded-2xl shadow-xl overflow-hidden">
                ...
                <div class="px-6 sm:px-8 py-5 border-b border-[#3a3a3a] bg-[#1a1a1a]/60">
                    ...
                    <p class="text-xs text-gray-400 mt-1">
                        Buka formasi baru untuk program magang BPS Kabupaten Bantul.
                        Untuk menambah divisi baru, gunakan menu
                        <a href="{{ route('divisi.index') }}" class="text-[#e88968] hover:underline font-medium">Manajemen Divisi</a>.
                    </p>
                </div>

                {{-- Form --}}
                <form action="{{ route('lowongan.store') }}" method="POST" class="p-6 sm:p-8 space-y-6">
                    @csrf
                        ...
                        <div class="relative">
                            <select name="divisi" id="divisi" required class="filter-input pr-10 appearance-none cursor-pointer">
                                ...
                                @foreach($divisiList as $divisi)
                                    <option value="{{ $divisi }}" class="bg-[#1f1f1f] text-white" {{ old('divisi') == $divisi ? 'selected' : '' }}>
                                        {{ $divisi }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <select name="status" id="status" required class="filter-input pr-10 appearance-none cursor-pointer">
                                    <option value="buka" class="bg-[#1f1f1f] text-white" {{ old('status', 'buka') == 'buka' ? 'selected' : '' }}>Dibuka</option>
                                    <option value="tutup" class="bg-[#1f1f1f] text-white" {{ old('status') == 'tutup' ? 'selected' : '' }}>Ditutup</option>
                                </select>
                                <div class="absolute inset-y-0 right-0 pr-3.5 flex items-center pointer-events-none text-gray-400">
                                    <i class="fas fa-chevron-down text-xs"></i>
                                </div>
                    ...
                    <div>
                        ...
                        <textarea name="deskripsi" id="deskripsi" rows="5" required
                            ...
                            placeholder="Jelaskan peran, tanggung jawab, dan gambaran umum pekerjaan peserta magang...">{{ old('deskripsi') }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="kualifikasi" id="kualifikasi" rows="4" required
                            class="filter-input resize-none"
                            placeholder="Sebutkan jurusan yang dicari, keahlian teknis (tools/software), dan kriteria lainnya...">{{ old('kualifikasi') }}</textarea>
                    </div>
