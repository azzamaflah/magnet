    ...
    <div class="claude-container">
        <div class="min-h-screen py-8 px-4">
            <div class="max-w-7xl mx-auto">
                ...
                @if($magang->is_hidden)
                    <div class="bg-yellow-500/15 border border-yellow-500/30 text-yellow-300 px-4 py-3.5 rounded-xl mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm">
                        <div class="flex items-center gap-3">
                            <i class="fas fa-eye-slash text-xl flex-shrink-0 text-yellow-400"></i>
                            <div>
                                <p class="font-semibold text-sm">Data Ini Sedang Disembunyikan</p>
                                <p class="text-xs text-yellow-200/80 mt-0.5">Data peserta ini tidak akan muncul di direktori data magang publik. Hanya Administrator yang dapat mengaksesnya.</p>
                            </div>
                        </div>
                        @if(auth()->check() && auth()->user()->isAdmin())
                            <form action="{{ route('magang.toggleVisibility', $magang) }}" method="POST" class="flex-shrink-0">
                                @csrf
                                @method('PATCH')
                                <button type="submit" class="px-3.5 py-2 bg-yellow-500 text-black font-semibold text-xs rounded-lg hover:bg-yellow-400 transition-colors inline-flex items-center gap-1.5 shadow">
                    ...
                    <div
                        ...
                        @if(auth()->user()->isAdmin() || (isset($magang->user_id) && $magang->user_id == auth()->id()))
                            <div class="flex flex-col md:flex-row gap-3 w-full md:w-auto">
                                ...
                                <form action="{{ route('magang.toggleVisibility', $magang) }}" method="POST" class="inline-block w-full md:w-auto">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="claude-button-secondary w-full justify-center {{ $magang->is_hidden ? 'text-yellow-400 border-yellow-500/40 bg-yellow-500/10' : '' }}">
                                        <i class="fas {{ $magang->is_hidden ? 'fa-eye' : 'fa-eye-slash' }}"></i>
                                        <span>{{ $magang->is_hidden ? 'Tampilkan ke Publik' : 'Sembunyikan' }}</span>
                                    </button>
                                </form>
                                @endif
