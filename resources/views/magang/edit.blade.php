                ...
                <form action="{{ route('magang.update', $magang) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="p-6 md:p-8 grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <select name="lowongan_id" id="lowongan_id" class="filter-select">
                                    ...
                                    @foreach($lowongans as $item)
                                        <option value="{{ $item->id }}" {{ old('lowongan_id', $magang->lowongan_id) == $item->id ? 'selected' : '' }}>
                                            {{ $item->judul_posisi }} ({{ $item->divisi }})
                                        </option>
                                    @endforeach
                                </select>
                                <span class="text-xs text-gray-400 mt-1 block">Divisi penempatan peserta magang ini.</span>
                            ...
                            <div>
                                ...
                                <input type="email" name="email" id="email" value="{{ old('email', $magang->email) }}"
                                    ...
                                    <div>
                                        ...
                                        <div class="date-input-wrapper">
                                            <input type="text" name="tanggal_mulai" id="tanggal_mulai"
                                                value="{{ old('tanggal_mulai', $magang->tanggal_mulai ? $magang->tanggal_mulai->format('Y-m-d') : '') }}"
                                                class="filter-input cursor-pointer" required>
                                    ...
                                    <div>
                                        ...
                                        <div class="date-input-wrapper">
                                            <input type="text" name="tanggal_selesai" id="tanggal_selesai"
                                                value="{{ old('tanggal_selesai', $magang->tanggal_selesai ? $magang->tanggal_selesai->format('Y-m-d') : '') }}"
                                                class="filter-input cursor-pointer" required>
                                ...
                                <div class="border-t border-[#3a3a3a] pt-4 mt-2">
                                    ...
                                    <label class="flex items-start gap-3 p-3.5 bg-black/20 border border-[#3a3a3a] rounded-xl cursor-pointer hover:border-[#d97757]/40 transition-all">
                                        <input type="checkbox" name="is_hidden" value="1" 
                                               {{ old('is_hidden', $magang->is_hidden) ? 'checked' : '' }}
                        ...
                        <div class="space-y-6">
                            ...
                            {{-- Upload Foto Profil --}}
                            <div>
                                <label for="foto" class="filter-label mb-2">Foto Profil</label>

                                @if($magang->foto)
                                    <div class="mb-3 flex items-center gap-3">
                                        <img src="{{ asset('storage/' . $magang->foto) }}" alt="Foto saat ini"
                                            class="w-20 h-20 object-cover rounded-xl shadow border border-[#3a3a3a]">
                                        <span class="text-xs text-gray-400">Foto profil saat ini tersimpan.<br>Unggah file baru hanya jika ingin mengganti.</span>
                                    </div>
                                @endif
                                ...
                                <img id="preview" class="preview-image" style="display: none;" alt="Preview Foto Baru">
                                @error('foto')
                                    <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                <label for="link_pekerjaan" class="filter-label mb-2">Link Karya (Opsional)</label>
                                <textarea name="link_pekerjaan" id="link_pekerjaan" rows="2"
                                    class="filter-input @error('link_pekerjaan') border-red-500 @enderror"
                                    placeholder="Contoh: https://drive.google.com/... atau https://github.com/...">{{ old('link_pekerjaan', $magang->link_pekerjaan) }}</textarea>
                                <span class="text-xs text-gray-400 mt-1 block">Tautan Google Drive, GitHub, atau portfolio online peserta.</span>
                    ...
                    <div class="bg-[#1a1a1a]/70 px-6 py-4 border-t border-[#3a3a3a] flex flex-wrap gap-3 justify-end items-center">
                        ...
                        <button type="submit" class="filter-btn filter-btn-primary">
                            <i class="fas fa-save mr-1.5"></i>Update Data Magang
                        </button>
                    </div>
                </form>
            </div>
